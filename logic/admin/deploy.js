...
const expand_deployment = async (json) => {
...
const prep_version = async (json) => {
    const version_hash = md5(json);
...
module.exports = {
    ...
    init: async (deployment) => {
        const json = JSON.stringify(deployment);
        await expand_deployment(json);
        const version = await prep_version(json);
        await db.query(`
    ...
    save: async () => {
        const json = await collapse_version();
        const version = await prep_version(json);
        await db.query(`
            INSERT INTO deployments (version_num, hash, date_deployed, data)
            VALUES ($1, $2, NOW(), $3)`,
